...
public class QueryEventRepositoryImpl implements QueryEventRepository, QueryEventOperations {
    ...
    @Override
    public List<EventTimeline> findSpaceViewsByCompanyAndDateRange(String companyReference, String startDate, String endDate) {
        Query query = new NativeSearchQueryBuilder()
            .withQuery(
                boolQuery()
                    .filter(byCompany(companyReference))
                    .filter(byType(SPACE_VIEW))
                    .filter(byTimestampBetween(startDate, endDate)))
    ...
    private Aggregations getAggregationsOf(SearchHits<Event> search) {
        return ofNullable(search.getAggregations())
            .orElseThrow(() -> new IllegalStateException("Aggregations are not found"));
    }
}
