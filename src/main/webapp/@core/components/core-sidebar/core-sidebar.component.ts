...
export class CoreSidebarComponent implements OnInit, OnDestroy {
  ...
  @Output()
  statusChangedEvent: EventEmitter<string>;
  ...
  set collapsed(value: boolean) {
    ...
    this.statusChangedEvent.emit('collapsed');
  }
  ...
  open(): void {
    ...
    this.statusChangedEvent.emit('opened');
  ...
  private _showOverlay(): void {
    ...
    this._animationPlayer = this._animationBuilder.build([animate('300ms ease', style({ opacity: 1 }))]).create(this._overlay);
  ...
  private _hideOverlay(): void {
    ...
    this._animationPlayer = this._animationBuilder.build([animate('300ms ease', style({ opacity: 0 }))]).create(this._overlay);
  ...
  close(): void {
    ...
    // Emit the 'openedChangedEvent' event
    this.openedChangedEvent.emit(this.isOpened);
    this.statusChangedEvent.emit('closed');
  ...
  event(event: string) {
    this.statusChangedEvent.emit(event);
  }
